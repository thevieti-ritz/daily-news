// ============================================
// WATCH.JS — Video Player Page Logic
// Handles video loading, likes, view count,
// comments, related videos, share modal
// ============================================
...
import {
  ...
  serverTimestamp, limit, where, arrayUnion, arrayRemove
...
async function loadVideo() {
  try {
    ...
    // Set page title
    document.title = `${video.title} — Daily News UG`;
    ...
    videoDate.textContent = video.createdAt ? video.createdAt.toDate().toLocaleDateString("en-UG", { year: "numeric", month: "long", day: "numeric" }) : "";
...
likeBtn?.addEventListener("click", async () => {
  ...
  if (hasLiked) {
    ...
    const current = parseInt(likeCount.textContent.replace(/[KM]/g, "")) || 0;
    likeCount.textContent = formatNumber(Math.max(0, (await getDoc(videoRef)).data().likes || 0));
  } else {
    hasLiked = true;
...
async function loadRelated(category, excludeId) {
  try {
    ...
    snap.docs.forEach(d => {
      ...
      relatedVideos.appendChild(createRelatedCard(v));
      count++;
    });

    if (count === 0) {
      // Load any videos if no related in same category
...
function createRelatedCard(video) {
  ...
  card.innerHTML = `
    <img class="related-thumb" src="${thumb}" alt="${escapeHtml(video.title)}"
...
async function loadComments() {
  try {
    ...
    snap.forEach(d => {
      ...
      commentsList.appendChild(createCommentEl(c));
    });
  } catch (err) {
...
function createCommentEl(comment) {
  ...
  const isAdmin = currentUser && currentUser.email === "dbernardinvestments@gmail.com"; // change to your admin email
...
async function postComment() {
  ...
  try {
    ...
    // Add optimistically
    const el = createCommentEl({ id: ref.id, ...commentData, createdAt: { toDate: () => new Date() } });
